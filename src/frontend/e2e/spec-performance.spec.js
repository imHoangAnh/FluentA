...
async function registerAndLogin(request) {
  const email = `performance+${crypto.randomUUID()}@example.com`
  const password = 'SecurePass123'

  const registerResponse = await request.post(`${apiBaseUrl}/api/v1/auth/register`, {
    data: { email, password, fullName: 'Performance Learner' },
  })
...
test('SPEC API p95 stays under 300ms for authenticated core reads', async ({ request }) => {
  ...
  for (let i = 0; i < 40; i += 1) {
    ...
    const response = await request.get(`${apiBaseUrl}/api/v1/auth/me`, { headers })
