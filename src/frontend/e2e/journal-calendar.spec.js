...
async function createJournal(page, headers, title, learningDate) {
  const response = await page.request.post('http://127.0.0.1:5000/api/v1/journals', {
    headers,
    data: { title, content: `<p>${title}</p>`, learningDate },
  });
...
test('Journal learning-date calendar opens populated dates and prepares empty dates', async ({ context, page }, testInfo) => {
  ...
  await createJournal(page, headers, 'No learning date note', null);
  ...
  const calendarResponse = await page.request.get(`http://127.0.0.1:5000/api/v1/journals/calendar?month=${monthInput()}`, { headers });
  expect(calendarResponse.status()).toBe(200);
  const calendarDays = (await calendarResponse.json()).data;
  expect(calendarDays).toEqual([{ date: populatedDate, count: 2 }]);
  const invalidResponse = await page.request.get('http://127.0.0.1:5000/api/v1/journals/calendar?month=June', { headers });
  ...
  await expect(page.getByTestId('journal-learning-date-input')).toHaveValue(emptyDate);
