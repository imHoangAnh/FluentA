...
const user = {
  id: 'kanban-redesign-user',
  email: 'kanban-redesign@fluenta.local',
  fullName: 'Kanban Learner',
...
test('Kanban uses the wide route and keeps extra-column overflow inside the board', async ({ page }, testInfo) => {
  ...
  await page.route('**/api/v1/**', async (route) => {
    ...
    if (request.method() === 'GET' && path.endsWith('/kanban/boards')) {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({
    ...
    if (request.method() === 'GET' && path.endsWith('/kanban/boards/board-1')) {
    ...
    if (request.method() === 'POST' && path.endsWith('/kanban/boards/board-1/columns')) {
    ...
    await route.fulfill({
      status: 503,
      contentType: 'application/json',
      body: JSON.stringify({ message: 'Deterministic Kanban redesign fixture' }),
    })
  })
  ...
  const wideLayout = await page.evaluate(() => {
    ...
    const workspace = document.querySelector('[data-testid="kanban-route-workspace"]').getBoundingClientRect()
    const board = document.querySelector('[data-testid="kanban-board-surface"]')
    const columns = [...document.querySelectorAll('.kanban-column-modern')].map((element) => element.getBoundingClientRect())
    return {
  ...
  await page.screenshot({ path: testInfo.outputPath('kanban-wide-three-columns.png') })

  await page.getByRole('button', { name: 'Add column' }).click()
  await page.getByTestId('kanban-column-name-input').fill('Review')
  ...
  await page.screenshot({ path: testInfo.outputPath('kanban-tablet-local-overflow.png') })
})
